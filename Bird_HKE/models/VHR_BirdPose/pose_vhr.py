...
# This file preserves upstream attribution and is modified to fit the VHR-BirdPose pipeline used here.
...
class VHRBirdPose(PoseHighResolutionNet):
    """HRNet + ViT (VHR-BirdPose)."""
    def __init__(self, cfg, **kwargs):
        super(VHRBirdPose, self).__init__(cfg, **kwargs)
        extra = cfg['MODEL']['EXTRA']

        vit_cfg = extra.get('VIT', {})
        self.vit = MyViTNet(
            depth=vit_cfg.get('DEPTH', 8),
    ...
    def init_weights(self, pretrained=''):
        super(VHRBirdPose, self).init_weights(pretrained)
