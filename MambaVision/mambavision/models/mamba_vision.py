...
class MambaVision(nn.Module):
    ...
    def __init__(self,
        ...
        if stage_dims is None:
            stage_dims = [int(dim * 2 ** i) for i in range(len(depths))]
        else:
            assert len(stage_dims) == len(depths), 'stage_dims must match depths length'

        if conv_stages is None:
            conv_stages = [True if (i == 0 or i == 1) else False for i in range(len(depths))]
        else:
            assert len(conv_stages) == len(depths), 'conv_stages must match depths length'

        if downsample_stages is None:
            downsample_stages = [True if i < 3 else False for i in range(len(depths))]
        else:
            assert len(downsample_stages) == len(depths), 'downsample_stages must match depths length'

        num_features = int(stage_dims[-1])
        self.num_classes = num_classes
        self.patch_embed = PatchEmbed(in_chans=in_chans, in_dim=in_dim, dim=stage_dims[0])
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]
        self.levels = nn.ModuleList()
        for i in range(len(depths)):
            conv = conv_stages[i]
            level = MambaVisionLayer(dim=stage_dims[i],
                                     depth=depths[i],
                                     ...
                                     downsample=downsample_stages[i],
                                     layer_scale=layer_scale,
...
def mamba_vision_CUSTOM(pretrained=False, **kwargs):
    custom_cfg = kwargs.pop('custom_cfg', {}) or {}

    depths = custom_cfg.get('DEPTHS', kwargs.pop('depths', None))
    num_heads = custom_cfg.get('NUM_HEADS', kwargs.pop('num_heads', None))
    window_size = custom_cfg.get('WINDOW_SIZE', kwargs.pop('window_size', None))
    dim = custom_cfg.get('DIM', kwargs.pop('dim', 80))
    in_dim = custom_cfg.get('IN_DIM', kwargs.pop('in_dim', 32))
    mlp_ratio = custom_cfg.get('MLP_RATIO', kwargs.pop('mlp_ratio', 4))
    drop_path_rate = custom_cfg.get('DROP_PATH_RATE', kwargs.pop('drop_path_rate', 0.2))
    stage_dims = custom_cfg.get('STAGE_DIMS', kwargs.pop('stage_dims', None))
    skip_first_two = custom_cfg.get('SKIP_FIRST_TWO_STAGES', kwargs.pop('skip_first_two_stages', False))

    if depths is None or num_heads is None or window_size is None:
        raise ValueError('CUSTOM build requires DEPTHS, NUM_HEADS, and WINDOW_SIZE in config.')

    if stage_dims is None:
        stage_dims_full = [int(dim * 2 ** i) for i in range(len(depths))]
    else:
        stage_dims_full = stage_dims

    if len(depths) != 4 or len(num_heads) != 4 or len(window_size) != 4:
        raise ValueError('CUSTOM build expects 4 stages for DEPTHS, NUM_HEADS, and WINDOW_SIZE.')
    if stage_dims_full is not None and len(stage_dims_full) != 4:
        raise ValueError('CUSTOM build expects 4 stages for STAGE_DIMS when provided.')

    if skip_first_two:
        stage_indices = list(range(2, len(depths)))
        depths_use = [depths[i] for i in stage_indices]
        num_heads_use = [num_heads[i] for i in stage_indices]
        window_size_use = [window_size[i] for i in stage_indices]
        stage_dims_use = [stage_dims_full[i] for i in stage_indices]
        conv_stages = [False for _ in stage_indices]
        downsample_stages = [True if i < 3 else False for i in stage_indices]
        model_dim = stage_dims_use[0]
    else:
        depths_use = depths
        num_heads_use = num_heads
        window_size_use = window_size
        stage_dims_use = stage_dims_full
        conv_stages = None
        downsample_stages = None
        model_dim = stage_dims_use[0]

    if pretrained:
        raise ValueError('CUSTOM build does not provide pretrained weights.')

    model = MambaVision(depths=depths_use,
                        num_heads=num_heads_use,
                        window_size=window_size_use,
                        dim=model_dim,
                        in_dim=in_dim,
                        mlp_ratio=mlp_ratio,
                        drop_path_rate=drop_path_rate,
                        stage_dims=stage_dims_use,
